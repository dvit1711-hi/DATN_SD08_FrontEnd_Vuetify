          ...
          <template #item.orderInfo="{ item }">
            <div>
              <div class="font-weight-bold">
                {{ getDisplayOrderCode(getRawItem(item)) }}
              </div>
...
const sortOrdersNewestFirst = (list = []) => {
  return [...list].sort((a, b) => {
    const timeDiff =
      parseOrderDateToTimestamp(b?.orderDate) -
      parseOrderDateToTimestamp(a?.orderDate);
...
const getOrderStatusColor = (order) => {
  ...
  if (stage === 'RETURNED') return 'error'
...
const currentTabOrders = computed(() => {
  return paymentTab.value === "online"
    ? onlineOrders.value
    : offlineOrders.value;
});
