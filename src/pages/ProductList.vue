  ...
  <div >
    <v-container class="py-8" fluid>
      ...
      <div class="list-toolbar mb-4">
        <div class="product-count">{{ filteredProducts.length }} sản phẩm</div>
      ...
      <div v-if="selectedFilterChips.length > 0" class="active-filter-bar mb-4">
        <button class="clear-all-chip" @click="resetAllFilters">Xóa lọc</button>
...
const filteredProducts = computed(() => {
  const filtered = products.value.filter((product) => {
    ...
    if (
      ...
      !activeFilters.value.selectedMaterials.includes(
        Number(product.materialID),
      )
    ) {
      return false;
    }
  ...
  switch (sortBy.value) {
    ...
    case "nameDesc":
      sorted.sort((a, b) =>
        String(b.productName || "").localeCompare(
    ...
    case "priceAsc":
      sorted.sort(
        (a, b) => Number(a.displayPrice || 0) - Number(b.displayPrice || 0),
      );
      break;
    case "priceDesc":
      sorted.sort(
        (a, b) => Number(b.displayPrice || 0) - Number(a.displayPrice || 0),
      );
      break;
    default:
      break;
...
const selectedFilterChips = computed(() => {
  ...
  pushMapped(
    activeFilters.value.selectedBrands,
    filterSources.value.brands,
    "brand",
  );
  pushMapped(
    activeFilters.value.selectedColors,
    filterSources.value.colors,
    "color",
  );
  pushMapped(
    activeFilters.value.selectedMaterials,
    filterSources.value.materials,
    "material",
  );
  pushMapped(
    activeFilters.value.selectedSizes,
    filterSources.value.sizes,
    "size",
  );
...
const quickAddSelectedVariant = computed(() => {
  ...
  return (
    quickAddActiveVariants.value.find(
      (item) =>
        Number(item.colorID) === colorId && Number(item.sizeID) === sizeId,
    ) || null
  );
});
...
const quickAddHasValidSelection = computed(() => {
  return (
    Boolean(quickAddSelectedVariant.value) && quickAddMaxQuantity.value > 0
  );
});
...
const removeFilterChip = (chip) => {
  if (chip.type === "brand") {
    activeFilters.value.selectedBrands =
      activeFilters.value.selectedBrands.filter(
  ...
  if (chip.type === "color") {
    activeFilters.value.selectedColors =
      activeFilters.value.selectedColors.filter(
        (v) => Number(v) !== Number(chip.value),
      );
  }
  ...
  if (chip.type === "material") {
    activeFilters.value.selectedMaterials =
      activeFilters.value.selectedMaterials.filter(
        (v) => Number(v) !== Number(chip.value),
      );
  }
  ...
  if (chip.type === "size") {
    activeFilters.value.selectedSizes =
      activeFilters.value.selectedSizes.filter(
        (v) => Number(v) !== Number(chip.value),
      );
  }

  if (chip.type === "price") {
    activeFilters.value.selectedPriceKeys =
      activeFilters.value.selectedPriceKeys.filter((v) => v !== chip.value);
  }
};
...
async function confirmQuickAddToCart() {
  ...
  try {
    ...
    showMessage(
      `Đã thêm "${quickAddDetail.value.productName}" vào giỏ hàng`,
      "success",
    );
    closeQuickAddDialog();
  } catch (error) {
...
const totalPages = computed(() => {
  const total = Math.ceil(filteredProducts.value.length / ITEMS_PER_PAGE);
...
watch(
  [sortBy, activeFilters, () => route.query.search],
  () => {
    currentPage.value = 1;
  },
  { deep: true },
