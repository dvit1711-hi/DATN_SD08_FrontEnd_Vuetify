          ...
          <v-menu offset-y :close-on-content-click="false" max-width="400">
            <template #activator="{ props }">
              <div v-bind="props" class="notif-trigger">
                ...
                <span v-if="totalUnread > 0" class="notif-trigger-badge">{{
                  totalUnread
                }}</span>
              </div>
            </template>
...
const { appTheme, toggleTheme } = useAppSettings();
...
const notifTabs = computed(() => [
  { value: "all", label: "Tất cả", icon: "", count: totalUnread.value },
  {
    ...
    label: "Đơn hàng",
    icon: "mdi-cart",
    count: countByType("order"),
  },
  {
    value: "review",
    label: "Đánh giá",
    icon: "mdi-star",
    count: countByType("review"),
  },
  {
    value: "stock",
    label: "Tồn kho",
    icon: "mdi-alert",
    count: countByType("stock"),
  },
  {
    value: "account",
    label: "Tài khoản",
    icon: "mdi-account-plus",
    count: countByType("account"),
  },
  {
    value: "return",
    label: "Trả hàng",
    icon: "mdi-keyboard-return",
    count: countByType("return"),
  },
]);
...
const loadUserInfo = async () => {
  try {
    ...
    const res = await axios.get(
      `http://localhost:8080/api/account/getById/${accountId}`,
      {
    ...
    if (account?.images) {
      if (
        account.images.startsWith("http") ||
        account.images.startsWith("data:image")
      ) {
...
const loadNotifications = async () => {
  try {
      ...
      const getQty = (p: any): number => {
        if (Array.isArray(p.productColors) && p.productColors.length > 0) {
          return p.productColors.reduce(
            (sum: number, pc: any) =>
              sum + Number(pc.stockQuantity ?? pc.quantity ?? 0),
            0,
          );
        }
        ...
        return Number(
          p.stockQuantity ?? p.totalQuantity ?? p.quantity ?? p.stock ?? 0,
        );
      };
    ...
    console.log(
      ...
      newOrderCount.value,
      "| Tồn kho:",
      outOfStockCount.value + lowStockCount.value,
      "| Review:",
      reviewCount.value,
      "| Tài khoản:",
...
.notif-panel {
  ...
  box-shadow:
    ...
    0 0 0 0.5px rgba(201, 169, 130, 0.08) inset;
}
