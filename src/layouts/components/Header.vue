  ...
  <div>
        ...
        <div class="d-flex align-center gap-4">
          <v-menu offset-y>
            <template v-slot:activator="{ props }">
              <div
                v-bind="props"
                class="d-flex align-center gap-2 text-caption"
        ...
        <div class="d-flex align-center gap-4">
          ...
          >
            <v-badge
              ...
              offset-y="8"
            >
              <v-icon size="28" style="color: #000000"
          ...
          <v-menu offset-y v-if="isLoggedIn">
            <template v-slot:activator="{ props }">
              ...
              >
                ...
                <v-icon v-else size="32" style="color: #000000"
    ...
    <v-toolbar color="#FFFFFF" height="50" class="px-8 border-bottom">
      <div class="d-flex align-center gap-6">
        <router-link
          :to="{ name: 'Home' }"
          class="text-decoration-none text-body2 font-weight-medium text-dark"
        >
          Trang Chủ
        </router-link>
        ...
        <router-link
          :to="{ name: 'Promotion' }"
          class="text-decoration-none text-body2 font-weight-medium"
          style="color: #ff6633"
        >
...
const loadCartCount = async () => {
  ...
  if (!Number.isFinite(cartId) || cartId <= 0) {
    ...
    if (Number.isFinite(accountId) && accountId > 0) {
      try {
        ...
        const resolvedCartId = Number.parseInt(
          cartRes.data?.id || cartRes.data?.cartID,
          10,
        );
  ...
  try {
    ...
    cartCount.value = (res.data || []).reduce(
      (sum: number, item: any) =>
        sum + (Number.parseInt(item.quantity, 10) || 0),
      0,
    );
  } catch (error) {
